...
## Tests
- `node --test build/test_shared_*.mjs`: 70 pass (with the shared-UI tests from `main`). That includes a blind bot within 2% of `blindOdds`.
...
Find it (merged into `main`) already calls the real modules, and its leak bot passes on this branch. The branch has `main` merged in, including the shared-UI work (`results`, `onboard`, `sfx`, `uistore`; doc sections 8–10), with no conflicts; 70 unit tests pass.
