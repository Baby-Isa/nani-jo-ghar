...
- **End-of-round screen** after every order and lab round; accuracy = the order's rows plus stray mistakes; mapped to the stars.
- **Picture tally**, top right: what you did, never the target.
- **Auto-tick at every level.** Count rows tick when their step closes, right or not.
...
- **Nani is a voice at stations.** Her card leaves the sidebar and the rows she names throb.
- **Onboarding scripts** (`data.onboard`) for the nine stations, via `js/shared/onboard.js`.
- **Skewer-count leak fixed:** one card per kind, titled as said ("ba ghos"), not one per skewer.
...
- **Chai tray:** water, milk and chai are taps (`Cook.Pour.measure`).
  One tap per line; rows tick when a cup is full; the hand star is the knob.
...
## Tests
All pass, one at a time, no console errors; screenshots checked: `--lab` (laptop every entry; level 2; phone; iPad), `--days 2 --canvas` (laptop, phone), `--open-kitchen 2`, `--orders`, `test_find.py`, shared Node tests.

`js/shared/` is untouched; only Cook's files are version-stamped.
...
- Q9, next wave: chop level 1 = one kind, serve reactions, serving the right person, a speaking moment.
