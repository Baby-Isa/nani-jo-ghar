...
**Built** (own files only, on the merged core host and rough art): `js/clinic/heal/games/{taste,fever,boing}.js`, `data/clinic/heal/*.json` (words through Cook's ids), `lab/clinic-heal-b.html`, `build/leak_clinic_heal_b.mjs`, `build/test_clinic_heal_b.py`, screenshots in `build/screenshots/clinic-heal-b/`.

- **taste (H5):** a tongue close-up with its own rack of 4 look-alike droppers and 3 cups (*limu, khun, loon, marcha; paani, dudh, chai*), taken in the called order; drop counts at level 3.
...
**Leak bot (blind, level 1):** taste 8.0–8.8% (exactly 1/12), fever ≤5.0%, boing ≤0.9%. Fair play wins 100% at every level; levels 2–3 are ≤0.8%.

**Browser:** 27/27 fair rounds pass (3 games × 3 levels × phone, iPad and laptop), with no console errors. The `--mistakes` run counts each deliberate mistake in the review.
