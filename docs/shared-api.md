...
|---|---|---|
| **Find it** | none left: phase 0–1 (on `main`) already calls the real modules | Done: `js/find/gen.js` uses `WhichOne.checkDecoys` / `rng` / `shuffle` and `Rel`, `mechanics/tell.js` uses `Say.tell` with `Find.fakeListen` as `speech` in the lab, `engine.js` does `Stars.installInto(Cook.data)`, and `round.js` scores `Stars.voice(moments, "find")`. Its leak bot passes on this branch |
