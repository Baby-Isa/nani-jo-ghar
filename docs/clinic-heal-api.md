...
## Host additions (core, 26 Sept; additive only)
The host is `js/clinic/heal/host.js`; the dev page is **`lab/clinic-heal-host.html`** (`?game=ear&level=2&side=left&kind=cousin&seed=7&quiet=1&onboard=0`; `&src=js/...` loads an extra script; `&tray=tweezers,cotton-bud,drops-blue` overrides the tray). It auto-loads every `js/clinic/heal/games/<id>.js` that exists and your `data/clinic/heal/<id>.json`. The reference game is `js/clinic/heal/games/cut.js`.
- **Coordinates.** `ctx.patient.hotspot(part, side)` returns `{x, y, r}` in **CSS px relative to `stage`** (its top-left), after any zoom. `part` may be `"knee"` or `"body-knee"`; sides are the patient's own. Recompute after `focus()` or a resize.
- **The patient** (a DOM/SVG figure, `js/clinic/figure.js`) is already inside `stage` when `mount` runs (`.cl-patient-layer`); put your own layer on top (`position:absolute; inset:0`). Extra calls: `ctx.patient.focus(part, side, zoom, ms)` → Promise (the close-up; `focus(null)` goes back), `mark(part, side, "plaster"|"bandage"|"cast"|"patch"|"cloth"|"blanket", {colour, turns, rotate})`, `swirl(part, side, on)`, `partAt(clientX, clientY, {active})` → `{part, side}`, `figure`. Moods beyond the contract's four: `sad`, `scared`, `sour`, `salty`, `yuck`, `ahh`, `hot`, `cold`, `idle`. Poses: `kick`, `kick-right`, `wave`, `shake`, `nod`, `hop`, `jump`, `jerk`, `shiver`, `stand`, `sit`.
- **The tray** is drawn by the host as the sidebar's column of dishes. `ctx.trayUI`: `onTap(fn(i, item, el))`, `select(i)`, `light(i)`, `used(i)`, `pulse(i, on)`, `dishes()` → elements, `hide()` (if you draw your own). A wrong item arrives as `{id, wrong: true}`: unusable, counted in the review.
- **Data.** `ctx.data` is your `data/clinic/heal/<id>.json` (or null). `ctx.ailment` = `{id, part, side, items?, ...}` (your data's `ailments[id]` merged). The default ailment is the last one whose `from` ≤ level; the default tray is `ailments[id].items`, else `def.itemsFor[ailmentId]`, else `def.items`.
- **Helpers.** `ctx.button(label, onPress, cls)` (the big button on the right; use it for Done), `ctx.after(ms, fn)` and `ctx.on(el, type, fn)` (cleared on destroy), `ctx.item(id)` / `ctx.icon(item, parent)` (the greybox or rough-art picture), `ctx.text(word, parent)` (Kutchi, placeholders grey italic; `"Ne poi [cloth]"` brackets = placeholder), `ctx.say(lineIdOrObject, {who})` (line ids from your data's `lines`, `{kutchi, english, who}`), `ctx.sfx(name)`, `ctx.signal(name)` (for your onboarding `wait`), `ctx.card.now(rowId)` / `addRow(row)` / `speak()`.
