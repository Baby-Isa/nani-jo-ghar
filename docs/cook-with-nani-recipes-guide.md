...
`{"n": …, "of": …}` is a number and a noun in the language's order; `"one": false` leaves out "one" ("chai", not "one chai"). Each spoken thing also becomes a row of the **order ladder** (`R.ladder(order)`): its dot (a step of the sequence), its group (`"seq"`, or `"any"`: items that share a dot), its quantity, who it's for, and "no" rows (no dot).

**`need`** (what the pantry step fetches) and **`steps`** (the chips on the mission card) are lists; `{"if": "dudh", "then": "cook-dudh"}` adds an item only sometimes.
...
Code never contains Kutchi or its grammar. Sentence frames are `lines` ("Muke {x} khape.", "Ne {x}."), and `grammar` says how they combine: number words (`numbers`), where the number goes (`count`: `"{n} {x}"`), how a list is said (`list`), which frame starts an order (`order`), and the "and then" linker (`then` → the draft line *Ne poi {x}.*, Mum to confirm; not spoken yet). Another language (Gujarati first) swaps `words`, `lines` and `grammar`.
