...
| **2. The tape, the table pool, speaking** | Yes (reads shared files if present) | Stage 2 (2a Measure up on a local `where` port or the shared one: tap the part, tap the mark, all taps); **T9 `dip`, T10 `stamp`, The pedal (`count` + a side tap), T1 `cut`**; 1a's L4 order form; the seven speaking moments on `say` (pills first: 2c, 1c, T15, 3d, 5c, 5b L3, 6a's *Achija*); 3d pass me in the line; swap `pick` and `say` for the shared ones when confirmed | Each new T-game in the lab at L1–3 with its own bot run and **the same gestures at every level (a test: the lab's level switch changes no input handler)**; phone 915×375 usable for cut and stamp (tolerance ≥ 28 px); `say` never blocks; the voice star only on a recognised or ticked answer |
| **3. Weather, photo, story, free play** | Shell files for integration only | 5b Going out with the curtain and walk-out; 5c Tell Ali; 5a's L3 family form; 6a's *How do I look?* beat and 6c at L2–3 (`pose`); Simba and Zazu as one-slot clients; the story routes (The spill, the Eid eve, Eid morning, the Ch4 door); the hub's 60-second route; the lookbook. **No new table games** (T2, T7, T8, T11 are merged or in Q.7) | `--days` plays Ch4, the eve and Ch5 end to end; `--morning 3` closes into the lookbook; the hedge strategy < 10%; no weather sound before Done; the daily route returns in ≤ 90 s |
...
| **5. Full body and the rest** | Own files + art | The full-body renderer (alignment test first), the mirror walk, the old photo route, and **only those Q.7 maybe-later games Zafar asks for** (none by default; T12 and T16 are T4's variants and arrive with T4's data) | Alignment within 4 px per base; each game its own bot run < 10% and a persona pass |
...
5. **Bandhani (T9) and block print (T10)** are cultural nods; the earlier restraint rule said "1–2 per rack view, never costume". Are they fine as table games? **Default: yes**, two of eight (after the quality pass), Kutch's own crafts, made by the child rather than worn as fancy dress.
...
8. **The shared `belt` is one gesture at every level** (Q.2). The clinic's P11 decision 5 defaults to tap at L1 and drag from L2, which UX §12 (26 Sept) no longer allows. **Default: tap at every level in both modes**; the order row is judged by tap order. This needs one line in the clinic's doc, which this pass does not edit.
