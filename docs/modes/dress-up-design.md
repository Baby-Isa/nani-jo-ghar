...
**Mechanics count after this design:** built and kept 7 (`wear, check, change, stitch, passme, count, say`-stub); new small 9 (`queue, tape, cut, seam, iron, pin, aim, dip, stamp, pose` — `pose` and `queue` are a few lines each); reused from Cook 5 (`fetch, count, pour, stir, fold` via fill-fold); shared 4 (`whichone`/`pick`, `where` from the clinic, `belt` from the clinic, `say`).
