...
**Date:** 25 Sept 2026
**Status:** a proposal for Zafar, revised on 25–26 Sept (the **Mini-game quality pass** and the **Pipeline design** at the top are current; Revision 3 was the clinic's deep dive under `docs/modes/DEEP-DIVE-BRIEF.md`). Nothing has been built. It follows `docs/modes/MODE-DESIGN-BRIEF.md` and builds on `docs/game-modes-v2.md` (mode 5), `docs/find-it-design.md` (the model), `docs/cook-with-nani-phase-a-design.md`, `docs/cook-with-nani-kutchi-audit.md`, `docs/cook-with-nani-build-log.md` and `docs/cook-with-nani-todo.md`.
...
**Why this section.** Zafar's late-evening steer (`docs/modes/MINIGAME-QUALITY-BRIEF.md`): the pipeline is the right idea, but the mode will live or die on its mini-games being *distinct and good*, and twenty healing games is too many. This section puts every mini-game of the pipeline design below through the five questions (what do you do, where is the challenge, where is the fun, where is the instruction, what is novel), scores it, cuts the healing library from twenty to **nine**, checks distinctness against the other five modes' pipeline designs, and writes a level-1 walkthrough for the first set of each stage. It **supersedes P4's grab row, P5's table and sets, P9's mechanics count, P11's decision 5 and P12's phases** where they differ (Q7 lists the edits, which are also made in place below). Everything else in the pipeline design stands. The Kutchi rule is unchanged: only the real words at the top of the pipeline design are Kutchi; everything in `[EN: …]` is a grey placeholder.
...
### Q2 What's working right now (2024–26), and the mechanic borrowed

P7's table (Toca Doctor, Toca Life Hospital, Dr. Panda Hospital, BabyBus, Pepi, My Town, Doctor Kids, Bimi Boo, Little Panda ToothBrush, the ear-doctor comedies, sushi-belt order games) still stands. Searched again 25–26 Sept 2026 for what is winning awards and charts *now*; the store pages are still blocked from here, so the rows use review and award pages.

| What's succeeding | The specific mechanic | Why it works | Borrowed where |
|---|---|---|---|
| **Sago Mini Jinja's Garden** (Apple Design Award 2025, Interaction) | One effortless gesture for the whole game, no reading, every touch answers | A 3–6-year-old learns the control once and never thinks about it again | Q1's rule: *tap the dish, tap the spot* in every healing game, one working gesture per game |
| **Toca Boca World** (the Hospital made free for everyone, 2025–26; the most-played kids' sandbox) | X-ray, casts, crutches, plasters as *visible states* the character keeps afterwards; everything reacts | Healing is a before/after picture the child can see, not a score | The healed states (plaster, cast, crutches, the patch) carried into the send-off and the album; every tap on the patient reacts (H13's wrong toe wiggles) |
| **Pok Pok** (Montessori playroom; PTPA and Common Sense favourites 2024–25) | Calm, hand-drawn, no fail, no levels; slow animations, gentle sound | Children stay regulated and come back | The no-fail rule (the belt loops; nothing is lost; wrong things just happen), the calm of the fever game as the end of a Busy morning |
| **Good Pizza, Great Pizza** (the enduring casual hit; a cult classic since 2014, still charting) | The customer *says* the order, sometimes plainly, sometimes in riddles; you build exactly that; the fun is deciphering | Listening is the game, not a gate before it | The prescription seam (P1): the doctor's spoken list is the belt's order, the tray's slots and the heal's steps; D3's clues at level 4 are the "riddle" orders |
| **Cooking Mama: Cuisine!** (Apple Arcade, 2022–24) | One gesture per mini-game, forgiving, "most are a satisfying breeze", only one (pasta cutting) needs finesse | Rounds feel good even when imperfect; mistakes cost little | No mid-round verdicts; the working gesture is easy (a lap, a swipe, a pull) and the *Kutchi* is the challenge |
| **Dr. Panda Hospital** (still the reviewers' reference doctor app) | Waiting room → bed → procedure → sticker; "learning sequencing" | The fixed order is what pre-schoolers learn first | The pipeline; the sticker album |
| **Toca Doctor** (Toca's original, still loved) | Pull the splinter along the arrow; bubbles up the tummy maze; 21 tiny puzzles, no text | A body as a set of small tactile toys | `pluck` (H3, H13), the ear cave; the bubbles go to *maybe later* (Q4) |
| **BabyBus Dental / Hospital** | Kill the bug, fit the resin shape; the animal *says* its complaint | The shape puzzle is a satisfying finish; a spoken complaint gives the child a reason | H4's `fill`; D2 |
| **Bluey / Hey Duggee apps** (one-touch, react-quick, more targets at higher levels) | One touch per target; difficulty is *how many*, never *how* | The control never changes | W4's two calls and the belt's speed knob: harder by count and time, same tap |
| **Overcooked / sushi-belt order games** | Items pass; grab what the order says; the belt loops | Time pressure without failure | P4's belt (tap only now) |

What none of them has, and the clinic keeps: one patient who reacts live to every touch, a doctor the children know, and the seam where *hearing Kutchi* decides who, where, what, how many and in what order.

### Q3 The stage mini-games: five questions, scores, verdicts

Scores 1–5 per question (do / challenge / fun / instruction / novel), then the sum out of 25. Gestures are the ones already in the pipeline design unless noted. All the stage variants are kept (each stage has ≤ 4), with two merges.

| Id | What do you do | Where is the challenge | Where is the fun | Where is the instruction (card and voice) | What is novel (in any mode) | Score | Verdict |
|---|---|---|---|---|---|---|---|
| **W1 Bring in the girl** | Tap the person the doctor names; they walk to the bench and greet him | The kind word among 2–3 kinds; nobody moves until tapped | The walk and the greeting (*Salamun alaykum*; the baby waves from Ma's lap) | Card: *[EN: Bring in] [EN: the girl]*; doctor's voice | Kinds of person, not names: *girl, boy, old man, old woman, baby* (Dress's bench calls family by name; Who's line-up is cats and family) | 5/4/3/4/4 = 20 | Keep |
| **W2 The old man in red** | Tap the one both words fit | Kind + colour or kind + *wadho/nindho*; decoys share one word | The wrong one shakes his head and sits (level 1 only); the right one stands slowly (elders) | *[EN: The old man] [EN: in red]* · *wadho [EN: boy]* | Two-word calls on *people*; shared with Dress 1a L3 on purpose (same `which` module) | 4/5/3/4/3 = 19 | Keep |
| **W3 You call them** | Say *{kind}, [EN: come]*; the one who matches what was heard stands | Production of the kind from a closed set of 3–5 | A wrong hearing: the wrong person stands, puzzled, sits | The card shows the kind's picture; the doctor nods at the bench | The speaking flip of the bench; shared shape with Dress 1c (*call them in*), reusing `tell` | 4/4/4/3/3 = 18 | Keep (level 2+) |
| **W4 Busy bench** | Tap two in the called order; comfort rings on everyone waiting | *Pela … ne poi …* over two kinds, against time | The rings: the baby's runs out first and Ma rocks him | *Pela [EN: baby], ne poi [EN: old woman]* | The only timer in the clinic; the order words on people | 4/5/4/4/4 = 21 | Keep (level 3) |
| **D1 Does it hurt here?** | Tap a pulsing part; the patient says *haa/nar*; on *haa* press **Found it**, on *nar* **Next** | Level 1 taught; from level 2 *haa* vs *nar* decides which button (a bot that presses Found it always fails every *nar*) | The *ding* and the swirl on a yes; the neutral face on both | *[EN: Does it hurt here?]*; patient: *haa* / *nar*; patient: *[EN: My knee]* | Yes/no as the *decision*, not the answer: the child acts on the word (Snap 5d and Who 3d borrow it, and say so) | 5/4/3/5/5 = 22 | Keep (the very first sessions) |
| **D2 It's my knee** | Card, quiet, the patient's line, tap the part; from level 2 say the part first (S2) | Which part (6 → 19), side in the patient's voice from level 3 | The giggle on a wrong tap at level 1; the swirl and *[EN: That's it]* | Patient: *[EN: My {part} hurts]* · *[EN: My left knee]* | Sides only ever in the patient's own voice (R3.2): the one place *my left* is heard from the person it belongs to. Dress 2a reuses `where` for the tape (deliberate) | 4/5/3/5/4 = 21 | Keep (the bulk) |
| **D3 The check-up detective** | Tap the kit item the doctor names, tap the part he names; the find shows only at the sore one; level 4 the patient's clues | Part + instrument per call; level 4 negation and position (*nar* my head; near my hand; up, not down) | The finds: a gurgle, a pink glow, *hot!*, a seed | *[EN: Listen to] [EN: the chest]* · *[EN: Look in] [EN: the ear]*; clues in the patient's voice | Two nouns per call (tool and part) and, at level 4, deduction from body words: Who did it? deduces from *things*, never body words (the border in P3 stands) | 4/5/4/4/5 = 22 | Keep (level 2+) |
| **P The belt** | Tap the asked item as it passes; it hops to the next empty dish; the doctor checks the tray | Noun, then colour/size, then count and order, against the belt's speed; look-alikes from level 2 | The hop into the dish, each item's own noise; the doctor lifting each dish and naming it | Card: *Muke [EN: plaster] khape* · *Muke limu khape, ne paani, ne [EN: honey]* · *Pela [EN: cotton], ne poi [EN: thread]* | A spoken list that becomes a physical tray whose order is then *worked through* at the next stage (the seam); the loop means nobody can lose. The clinic owns `belt`; Tidy 2c, Dress 3b, Snap 1c and Who 2b reuse it, tap at every level | 5/5/4/5/5 = 24 | Keep; **tap at every level** |
| **E1+E2 Which face?** | Tap the face card that matches what the patient said; a sad answer gets one more thing, then the question again | 2 cards at level 1 (happy/sad), 4 at level 2, 5 at level 3 with a *was/now* pair (scared → happy after the boing) | The one-more-thing (Big Ma's song, the lollipop); the patient's face changing to match | *[EN: Is everything okay now?]*; patient: *[EN: Yes! Now I'm happy]* · *[EN: Still a bit sad]* | Feelings as the *patient's* report the child must read back; merged into one `feel` ladder (E1 is E2's level 1). Tidy 5c and Monsoon S5b share the feelings words on purpose, not this mechanic | 5/4/4/4/4 = 21 | Keep (merged) |
| **E3 Say goodbye** | Say the goodbye the doctor cues; the patient waves back | Closed set of 3: *Achija!* / *Aabhar aanjo!* / *[EN: Get well soon]* | The wave and the walk out on crutches | *[EN: Say thank you to the doctor]* · *[EN: Say bye]* | The cued goodbye (which of three) rather than a free one; `tell` | 4/3/3/4/3 = 17 | Keep (level 2+) |
| **E4 You ask** | Say *[EN: How do you feel?]*; the patient answers; tap the face | Production of the question frame, then E2's listening | The patient waits for the question, eyebrows up | The card shows the question | The child *asks* and then must hear the answer: two rows in one exchange | 4/4/3/4/4 = 19 | Keep (level 3) |
...
### Q5 Level-1 walkthroughs: the first set of each stage

**W1 Bring in the girl.** The waiting-room bench by the door, two people on it (a girl and a boy, seats shuffled), the doctor by the examination bench with his hands folded. The request card: his face, *[EN: Bring in] [EN: the girl]*, each chunk lighting as he says it; it shrinks into the left sidebar. Nothing on the bench moves. The child taps the girl: she stands, walks to the bench, *Salamun alaykum*; the doctor, *Wa alaikum salaam*; the card's line ticks. (A tap on the boy at level 1: he shakes his head and sits; the doctor says the line once more, the gentle correction.) The big button on the right, *[EN: Where does it hurt?]*, throbs; the child presses it. Ten seconds.

**D1 Does it hurt here?** The girl on the bench, symmetrical, hands in her lap; three parts pulse gently (knee, hand, ear). The card: *[EN: Does it hurt here?]* in the sidebar; the doctor's voice. The child taps the hand: the patient, neutral, *nar*. Taps the knee: *haa*, a *ding*, the sore swirl appears, and she says *[EN: My knee]*. The **Found it** button on the right lights; the child presses it; the doctor names the ailment, *[EN: A scrape.] Muke [EN: plaster] khape*, and that line becomes the next card. Nothing here is graded at level 1; it teaches *haa*, *nar* and the part's name.

**The belt (pharmacy).** The counter, the dispensary shelves behind, the doctor behind the counter; the request card, *Muke [EN: plaster] khape*, shrinks into the sidebar; the tray at bottom right shows **one** empty dish. The belt starts: six dishes ride past right to left, one every 2.5 s (the plaster, a bandage, a bottle, a cloth, a spoon, a fan; no look-alike of the plaster at level 1). The child taps the plaster: it hops into the dish with a plink; the card's line ticks. The doctor lifts the dish and names it, *[EN: The plaster.] Achija.* Big button: *[EN: To the bench]*.

**H2 Wash, stitch, plaster (the first-ever healing game).** The girl's knee in close-up, a pink swirl; the tray in the sidebar as a column of three dishes: the jug, the cloth, the plaster (in that order). The card shows one line at a time. *Pela paani*: the jug's dish throbs after a moment; the child taps it (it lifts), taps the knee: water pours the right amount, the ring goes green, *[EN: cold!]*; the line ticks and the jug's dish stays as a tick. *Ne poi [EN: cloth]*: tap the cloth, tap the knee: a dab; tick. *Ne poi [EN: plaster]*: three designs slide up; the child taps one (a cat), taps the knee: it sticks, the girl looks down at it and grins; tick. The **Done** button; the girl stands with her cat plaster on. Under a minute.

**H1 The kicking knee (session 2's healing game).** Ali on the bench, a bump on the knee; the tray: the hammer, a bandage. *[EN: Tap the knee]*: tap the hammer, tap the knee: the leg kicks, the dishes rattle, Kasuku squawks, Ali laughs; tick. *[EN: Bandage]*: tap the bandage, drag round the knee: one lap, one turn, the badge shows 🩹 1, 2, 3 (any number at level 1); tap Done: the line ticks as the bandage is put down. Ali stands with a wrapped knee.

**H3 The seed in the ear.** The cousin, one ear glowing pink; the tray: torch, tweezers, bud, drops. *[EN: Look in the ear]*: tap the torch, tap the ear: the cave opens with one sesame seed in it. *[EN: Pull it out]*: tap the tweezers, drag the seed out along the arrow: a *pop*, *[EN: that tickles!]*; tick. *[EN: Clean it]*: tap the bud, tap the canal a few times (no count at level 1); tap the drops, tap the ear once; Done. The cousin shakes his head and hears again.

**H5 The taste test.** A tongue striped in three colours. *[EN: Stick out your tongue]*, then *Pela limu*: three identical droppers on the tray; the child taps the one the card names (its dish throbs after 8 s if nothing happens), taps the tongue: the whole screen puckers, a stripe clears; tick. (Level 1 stops at one taste.) *Paani!*: tap the cup, tap the mouth: a rinse and a spit into the bowl; everyone laughs; Done.

**H6 Too hot, just right.** Nana on the bench, red-cheeked; the tray: the thermometer, the cloth, a blanket, the fan. *[EN: Take the temperature]*: tap the strip, tap his forehead; the doctor: *[EN: Hot!]*; tick. The card: *[EN: Cloth]*; tap the cloth, tap the forehead; Nana sighs. *[EN: How do you feel?]*; Nana: *[EN: just right]*; the Done button lights; the child presses it. One exchange at level 1.

**E1 Which face?** The girl stands, healed, her plaster on. The doctor: *[EN: Is everything okay now?]* The girl: *[EN: Yes! Now I'm happy]*. Two big face cards on the right (happy, sad); the child taps the happy one; she beams, *Aabhar aanjo!*; a sticker of her goes into the album; then the end-of-round screen: the stopwatch, four accuracy slots (the bench, the belt, the heal's three lines pooled as one, the face), the hints badge; *Next*; the word review: *girl, knee, plaster, paani, happy*.

### Q6 The Sceptic on the pass

The auto-tick could have leaked counts; it doesn't, because a count line ticks when the item is *put down*, not on the Nth action. A wrong thing happening without comment could have let a child "try everything"; it doesn't win anything, because every wrong thing is a red slot in the review and the accuracy badge is what turns gold. The blind-bot estimate for a level-1 patient is unchanged from P5 (≈ 0.5% for the ear), and the belt's loop still means nothing is lost. New bot strategies for the leak report: *tap every dish in tray order* (H2, H5), *drag until the tick* (H1: it never ticks on its own), *blanket every answer* (H6), *press Found it always* (D1b, kept), *grab all* and *first past* (the belt, kept).

### Q7 What this changes in the pipeline design below

| Where | Change |
|---|---|
| P4 the grab row | Tap at every level; the level-3 order row is the order of taps |
| P5 intro and table | Twenty → nine; sets 1 and 2 as in Q4; set 3 removed; H14 into H1, H8 into H2; gestures per game as in Q1 |
| P8 story | The wet neighbour goes D2 → H6 (H11 cut); Nani's sore throat is Ch5's in Cook (H7 cut); the hen's thorn is H13 |
| P9 mechanics count | New: `call, probe, clue, belt, pluck, brush, fill, taste, warm, chart, xray, feel, tap` (13); `bubbles`, `turn` dropped; Monsoon's `cover` no longer needed |
| P11 decision 5 | Overturned: tap at every level |
| P12 phases | Phase 1 heal set 1 = H2/H8, H1, H3, H5, H6; phase 2 set 2 = H4, H9, H12, H13; phase 4 is art only (no set 3); the leak strategies in Q6 |
| P10 words | Rows for H7, H11, H17–H20 (honey, tissues, steam, cream, oil, comb, shampoo, *bend it*, *look up/down*, *breathe in/out*) drop to priority 3 |
...
**Sets after the quality pass (the "Set" column above is superseded):** **set 1** (phase 1) = H2/H8, H1, H3, H5, H6, which covers count, colour, side, order, hot/cold, speed and the real food words, and gives Layla the two she'll replay (the taste test, the kicking knee); **set 2** (phase 2) = H4, H9, H12, H13 (directions, spoken numbers, the other side, size × side). **There is no set 3**: H7, H10, H11, H15–H20 are cut or on the maybe-later list (Q4), and H14 is inside H1.
...
**Mechanics, counted.** Kept from phase 1: `where, check, stick, wrap, lift, tuck, drops, handover, ask, tell, you` (11). Reused from Cook: `pour, add, stir, count, knead, passme`, the tadka and Chai tray stations (6 + 2). Shared: `tell`, `which`, **Dress up's `stitch`** (H2/H8). New, after the quality pass: `call` (W1–W4), `probe` (D1), `clue` (D3 level 4), `belt` (P4, tap at every level), `pluck` (H3, H13), `brush` (H4), `fill` (H4), `taste` (H5), `warm` (H6, from R3), `chart` (H12), `xray` (H1's break), `feel` (E1–E4), `tap` (the hammer's kick, H1) (13). `bubbles` and `turn` went with H10 and H18; Monsoon's `cover` is no longer needed (H11 cut). `care` retired.
...
5. **The belt's grab: tap or drag?** ~~Default: tap at level 1, drag from level 2~~ **Decided by UX §12 and the quality pass (Q1): tap at every level**; the belt loops (a sushi belt), the belt stopper is a hint that costs the tick.
