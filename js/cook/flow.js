...
(function (global) {
  ...
  async function runOrder(order, day, { demo } = {}) {
    ...
    // the order comes up big in the middle while it's said, then flies into the sidebar
    UI.hideBubble();
    UI.mission.open({ who, name, ladders, line: ctx.orderLine, busy: Cook.save.mode === "busy" });
  ...
  async function runLab(key, guided, { level = Cook.labLevel || 1, region } = {}) {
    ...
    const openCard = (what, steps) => {
      ...
      ctx.lines = [{ line: ctx.orderLine }];
      UI.mission.open({ who: "nana", name: "Nana (lab)", ladders: ctx.ladders, line: ctx.orderLine, busy: Cook.save.mode === "busy" });
