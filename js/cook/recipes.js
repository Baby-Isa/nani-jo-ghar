 ...
 * ("seq" | "any"), quantity, who it's for, and "no" rows.
 */
(function (global) {
    ...
    const walk = (e, forWho) => {
      ...
      if (e.forEach) {
        ...
        list.forEach((it, j) => {
          ...
          (e.say || [Object.assign({}, e, { forEach: undefined })]).forEach((sub) => walk(sub, f));
        });
        env.vars.it = saved;
        return;
      }
      if (e.list) {
        // a spoken list; each id its own dot, an any-order group shares one
        const series = seriesOf(def, e.list, env);
        const line = Lang.list(series.flat());
        lines.push(line);
        let p = 0;
        series.forEach((entry) => {
          dot++;
          [].concat(entry).forEach((id) => rows.push({ kind: "item", ids: [id], qty: 1, dot, group: Array.isArray(entry) ? "any" : "seq", for: forWho, line: line.parts[p++] }));
        });
        return;
      }
      if (e.tally) {
        ...
        if (!ls.length) return;
        lines.push(ls.length > 1 ? Lang.join(ls) : ls[0]);
        dot++;
        ids.forEach((id, j) => rows.push({ kind: "item", ids: [id], qty: t[id], dot, group: "any", for: forWho, line: ls[j] }));
        return;
      }
      ...
      rows.push({ kind, ids: ps.filter((x) => typeof x === "string"), qty: ps.find((x) => typeof x === "number") || 1, dot: kind === "no" ? null : dot, group: "any", for: forWho, line });
    };
    (def.say || []).forEach((e) => walk(e, e.for ? res(e.for, env) : undefined));
    return { lines, rows };
  }
